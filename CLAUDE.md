...
Mise à jour côté appareil (vérifié le 22/09/2026) : le service worker
(`registerType: 'autoUpdate'`, `registerSW` dans `main.tsx`) détecte la nouvelle version à
l'ouverture suivante de l'appli, l'installe et recharge la page automatiquement : un seul
lancement suffit. Une appli laissée ouverte pendant le déploiement ne change pas d'elle-même
(pas de vérification périodique). Contrôle : Réglages, ligne « Version » (injectée depuis
`package.json`) ; penser à `npm version x.y.z --no-git-tag-version` à chaque évolution
visible.
